import pandas as pd
from building_data.osm import get_buildings
from building_data.tabula import get_heat_demand_per_m2, classify_osm_building

def load_district(place: str) -> pd.DataFrame:
    """
    ...
    buildings["building_type"] = buildings["building"].apply(
        lambda x: classify_osm_building(str(x)) if pd.notna(x) else "MFH"
    )
